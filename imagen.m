function [rxData, txData, bandera] = imagen(fileTx, txGain)
...
elseif ~license('test', 'WLAN_System_Toolbox') % Verifique que haya una licencia válida
   error('Se requiere una licencia valida para WLAN Toolbox para ejecutar este ejemplo.');
end
...
spectrumScope = dsp.SpectrumAnalyzer('SpectrumType', 'Power density', 'SpectralAverages', 10, 'YLimits', [-130 -50], 'Title', 'Espectro de senial WLAN de banda base recibida', 'YLabel', 'Densidad espectral de potencia','Position', [69 376 800 450]);

% Configurar el visor de diagramas de constelaciones para símbolos WLAN ecualizados
constellation = comm.ConstellationDiagram('Title', 'Simbolos de WLAN ecualizados', 'ShowReferenceConstellation', false, 'Position', [878 376 460 460]);
...
numMSDUs = ceil(length(tximagen)/msduBits);
padZeros = msduBits-mod(length(tximagen),msduBits);
txData = [tximagen; zeros(padZeros,1)];
...
sdrTransmitter.CenterFrequency = 2.432e9;  % Canal 5
...
fprintf('\nGenerando forma de onda de transmision WLAN:\n')
...
while (searchOffset + minPktLen) <= rxWaveformLen
    ...
    if isempty(pktOffset) || (pktOffset+double(indLSIG(2))>rxWaveformLen)
        if pktInd==1
            disp('** No se detecto ningun paquete **');
        end
        break;
    end
    ...
    fprintf('\nPaquete-%d detectado en el indice %d\n',pktInd,pktOffset+1);
    ...
    if failCheck
        fprintf('  Fallo de verificacion L-SIG \n');
        searchOffset = pktOffset+1.5*lstfLen;
        continue;
    else
        fprintf('  Aprobacion de verificacion L-SIG \n');
    end
    ...
    [rxBit{pktInd},crcCheck] = fcsDetector(double(rxPSDU));

    if ~crcCheck
         disp(' Verificacion de MAC CRC');
    else
         disp('  Error de verificacion MAC CRC');
    end
    ...
    if displayFlag
        ...
        fprintf('                            MCS: %d\n',lsigMCS);
        fprintf('                         Longitud: %d\n',lsigLen);
        fprintf('    Numero de muestras en el paquete: %d\n\n',rxSamples);
        ...
        fprintf('    Secuencia de numeros:%d\n',packetSeq(pktInd));
    end
...
if ~(isempty(fineTimingOffset)||isempty(pktOffset))&& (numMSDUs==(numel(packetSeq)-1))
    ...
    fprintf('          Numero de errores en los bits = %d.\n', err(2));
    fprintf('  Numero de bits transmitidos = %d.\n\n',length(txData));

    % Recreate imagen from received data
    fprintf('\nConstruyendo una imagen a partir de los datos recibidos.\n');

    str = reshape(sprintf('%d',rxData(1:length(tximagen))),8,[]).';
